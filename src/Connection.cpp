...
void Connection::writecallback()                   
{
    ...
    {
        const ssize_t writen=::send(fd(),outputbuffer_.data(),outputbuffer_.size(),0);    // 尝试把outputbuffer_中的数据发送出去。
        if (writen>0)
        {
        ...
        if ((writen==-1)&&((errno==EAGAIN)||(errno==EWOULDBLOCK))) break;   // 内核发送缓冲区满，等待下次可写事件。

        errorcallback();    // 其它错误，按连接异常处理。
        return;
    }
