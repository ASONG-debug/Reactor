...
// 用eventfd唤醒事件循环线程。
 ...
 void EventLoop::handlewakeup()
 {
    ...
    {
        std::lock_guard<std::mutex> gd(mutex_);      // 仅在搬运任务时持锁，避免执行任务期间阻塞生产者。
        std::swap(localtasks,taskqueue_);
    }

    while (localtasks.empty()==false)
    {
        std::function<void()> fn=std::move(localtasks.front());
        localtasks.pop();
        fn();
    }
 }
