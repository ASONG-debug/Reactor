...
void Buffer::append(const char *data,size_t size)             
{
 ...
 void Buffer::appendwithsep(const char *data,size_t size)  
 {
    ...
    else if (sep_==1)     // 四字节的报头。
    {
        ...
        buf_.append(reinterpret_cast<const char*>(&len),sizeof(len));           // 处理报文长度（头部）。
        buf_.append(data,size);                    // 处理报文内容。
    }
    // 其它的代码请各位自己完善。
 }
...
bool Buffer::pickmessage(std::string &ss)                           
{
    ...
    {
        if (buf_.size()<sizeof(uint32_t)) return false;

        uint32_t len=0;
        memcpy(&len,buf_.data(),sizeof(uint32_t));             // 从buf_中获取报文头部。
        len=ntohl(len);

        if (buf_.size()<len+sizeof(uint32_t)) return false;     // 如果buf_中的数据量小于报文头部，说明buf_中的报文内容不完整。

        ss=buf_.substr(sizeof(uint32_t),len);                        // 从buf_中获取一个报文。
        buf_.erase(0,len+sizeof(uint32_t));                          // 从buf_中删除刚才已获取的报文。
    }
